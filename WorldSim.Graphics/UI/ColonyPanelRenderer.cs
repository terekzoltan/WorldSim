...
public sealed class ColonyPanelRenderer
{
    ...
    private static string BuildSupplySegment(ColonyHudData colony)
    {
        if (colony.InventoryCapacityBonusSlots <= 0 && colony.InventorySupplyEfficiencyMultiplier <= 1.0001f)
            return string.Empty;

        return $", Supply[cap+{colony.InventoryCapacityBonusSlots} effx{colony.InventorySupplyEfficiencyMultiplier:0.00}]";
    }
}
