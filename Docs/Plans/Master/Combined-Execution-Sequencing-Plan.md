...
| Track B agent | Step10B.5-F2-A runtime war mobilization / launchable warrior availability | F1 ✅ | ⬜ First behavior-fix slice. Make hostile/war pressure create an organic path to enough launchable warriors after home-defense reserve, without direct campaign creation, score-threshold tuning, scout-policy change, or AI/App/Graphics changes. Track B may run mini-SMR for local proof only. Detail: `Docs/Plans/Master/Wave10-Step10B5-F2A-Runtime-War-Mobilization-Launchability.md`. |

| Track B agent | Step10B.5-F2-B mini-SMR / hostile lifecycle harness confidence | F2-A focused tests/build green | ⬜ Small Track-owned SMR/harness gate to prove the F2-A runtime fix moved the main-run hostile lifecycle past `no_available_warriors_after_home_defense`. Mini-SMR only: small seed/planner/config count, local raw artifacts, no full closeout recommendation. Detail: `Docs/Plans/Master/Wave10-Step10B5-F2B-Mini-SMR-Harness-Confidence.md`. |

| Track B agent | Step10B.5-F2-C target-knowledge / scout gate policy fix | F2-A/F2-B show target knowledge or scout gate as next blocker | ⬜ Conditional minimal runtime fix. Default policy remains: `War` targets may be baseline-known for first organic launch; scout intel remains a quality/target-choice signal. Preserve caps, home-defense, pair-cap, route preflight, and side-probe provenance. Detail: `Docs/Plans/Master/Wave10-Step10B5-F2-Target-Knowledge-Policy-Fix.md`. |

| Track B agent + SMR Analyst support | Step10B.5-F3 hostile organic pilot/confirm | F2-A/F2-B ✅ and F2-C if needed ✅ | ⬜ Run staged pilot/confirm before any full 90-run package. Stop if launches remain zero and return diagnostic RED; continue only if hostile organic launches in multiple seed/planner combinations or a new explicit blocker is identified. Detail: `Docs/Plans/Master/Wave10-Step10B5-F3-Hostile-Organic-Pilot-And-Confirm.md`. |
