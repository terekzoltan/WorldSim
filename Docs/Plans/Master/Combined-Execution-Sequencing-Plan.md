...
| Track B agent | Step10B.5-F5 stress seed-606 survival repro/fix | F4 accepted ✅ + Meta keeps stress seed-606 active | ⬜ Unblocked next Track B step under normal stress seed-606 scope only. The old F3 standard `movementSpeedMultiplier=0` collapse is not valid F5 evidence. Reproduce/classify/fix only the three known Step10B.2 stress seed-606 survival lanes; do not weaken `SURV-*` assertions, do not broaden to full stress/hostile/pure/perf packages, and do not use F5 for manual downstream positive proof. Detail: `Docs/Plans/Master/Wave10-Step10B5-F5-Stress-Seed606-Survival-Repro-Fix.md`. |
