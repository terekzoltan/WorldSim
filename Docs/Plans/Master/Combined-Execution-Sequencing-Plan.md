...
| Track B agent | Step10B.5-F4 manual downstream diagnostics | Route C accepted ✅ | ⬜ Unblocked next Track B step. Explain convoy/scout/siege-unit non-activation under manual lifecycle with reason counters before changing behavior. Do not claim no-tech runs prove dedicated siege-unit failure; distinguish convoy requested/spawned/delivered/failed. Detail: `Docs/Plans/Master/Wave10-Step10B5-F4-Manual-Downstream-Diagnostics.md`. |

| Track B agent | Step10B.5-F5 stress seed-606 survival repro/fix | F4 result reviewed and Meta keeps stress seed-606 active | ⬜ Deferred/narrowed. The old F3 standard `movementSpeedMultiplier=0` collapse is not valid F5 evidence. Only the separate stress seed-606 survival issue may justify F5, and only after F4 result review keeps it active. Do not weaken survival assertions. Detail: `Docs/Plans/Master/Wave10-Step10B5-F5-Stress-Seed606-Survival-Repro-Fix.md`. |
...
- Route C is accepted: F4 is explicitly unblocked next; F5 remains deferred/narrowed behind F4 review and a separate Meta decision on stress seed-606 survival scope; F6 and full hostile packages remain blocked until later Meta/SMR decision.
