...
| Active major/epic exclusivity | New major/epic beats rejected when matching active severity exists. | `error activeMajorStoryBeatConflict(...)` and `error activeEpicStoryBeatConflict(...)` exist for explicit core story severity plus RFM-D2-authorized active beat facts. | `model.problem`; `design.problem`; `DirectorRuntimeAssertionsMapper.appendActiveBeat`; `DirectorOutputAssertionsProblemMapper`; `DirectorRefineryPredicatePromotionTest`; `DirectorModelValidator.hasActiveSeverity` | real formal predicate/error predicate + transitional Java guard + mapped runtime fact | Formal artifact and Java validator | Formal coverage is intentionally narrow: it covers explicit core story severity against active beat severity/remainingTicks. It does not prove Java effect-count severity inference, nested effect semantics, budget, domain stack, campaign, or causal-chain coverage. | RFM-D3 promoted this family first because it is the smallest runtime-fact-backed formal rule beyond cooldown that does not require nested effect/bias modeling. |
...
| `INV-08` | active major exclusivity | real formal predicate/error predicate + transitional Java guard + mapped runtime fact | RFM-D3 covers explicit core major story severity against RFM-D2-authorized active major beat facts; Java effect-count inference remains Java-held. |
| `INV-09` | active epic exclusivity | real formal predicate/error predicate + transitional Java guard + mapped runtime fact | RFM-D3 covers explicit core epic story severity against RFM-D2-authorized active epic beat facts; Java effect-count inference remains Java-held. |
...
RFM-D3 implementation selected active major/epic exclusivity only after RFM-D2 and Meta review. This was accepted even though it was not first in the original priority list because it is the smallest runtime-fact-backed formal promotion that avoids nested effect/bias model expansion.
...
## RFM-D4 Differential Harness Evidence

`DirectorRefineryDifferentialHarnessTest` is a test-owned differential report harness. It does not add production diagnostics, runtime behavior, ScenarioRunner evidence, or new `.problem` predicates. Its purpose is to keep the current formal-vs-validator-vs-bridge boundary explicit.

| Differential category | Meaning in RFM-D4 | Representative covered cases |
|---|---|---|
...
RFM-D4 does not claim solver parity for nested effects/biases/campaign/causal-chain semantics. These remain explicit unsupported/formal-gap surfaces until a later approved predicate-promotion or family-fidelity slice opens them.
