...
public abstract class Animal
{
    ...
    protected Animal((int x, int y) pos, int speed, int vision, Random rng)
    {
        Pos = pos;
    ...
    public static Animal Spawn((int x, int y) pos, Random rng)
    {
        // Simple ratio: 70% herbivores, 30% predators
        return rng.NextDouble() < 0.7
            ? new Herbivore(pos, rng)
            : new Predator(pos, rng);
    }
...
public sealed class Predator : Animal
{
    ...
    public Predator((int x, int y) pos, Random? rng = null) : base(pos, speed: 2, vision: 6, rng ?? Random.Shared) { }
